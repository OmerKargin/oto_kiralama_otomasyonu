...
namespace oto_kiralama_otomasyonu
{
    public partial class arac_islemleri : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (cevap == DialogResult.Yes && dataGridView1.CurrentRow.Cells[0].Value.ToString().Trim() != "")
            {
                try
                {
                    ...
                    OleDbCommand sil = new OleDbCommand("delete from arac where plaka='" + dataGridView1.CurrentRow.Cells[1].Value.ToString() + "'", baglanti);
                    sil.ExecuteNonQuery();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                OleDbCommand komut = new OleDbCommand("insert into arac(plaka,marka,model,arac_tipi,renk,yil,resim,durum,arac_durum,satis_durum) values('" + textBox1.Text + "','" + textBox2.Text + "','" + textBox3.Text + "','" + textBox4.Text + "','" + comboBox1.Text + "','" + textBox5.Text + "','" + Path.GetFileName(yeniyol)+ "','"+ "Uygun" + "','"+comboBox2.Text+"','"+comboBox3.Text+"')", baglanti);
                //
                // 
                if (baglanti.State == ConnectionState.Closed)
                {
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            ...
            OleDbDataAdapter adtr = new OleDbDataAdapter("select * From arac where plaka like'%" + textBox6.Text + "%'or marka like '%" + textBox6.Text + "%'or model like '%" + textBox6.Text + "%'or arac_tipi like '%" + textBox6.Text + "%'or renk like '%" + textBox6.Text + "%'", baglanti);
            adtr.Fill(dt);
            dataGridView1.DataSource = dt;
            adtr.Dispose();
        ...
        {
            OleDbCommand komut = new OleDbCommand("update arac set plaka='"+textBox1.Text+"',marka='"+textBox2.Text+"',model='"+textBox3.Text+"',arac_tipi='"+textBox4.Text+"',renk='"+comboBox1.Text+"',yil='"+textBox5.Text+"',resim='" + Path.GetFileName(yeniyol) + "' where plaka='"+ dataGridView1.CurrentRow.Cells[1].Value.ToString() + "'", baglanti);
            //
            // 
            if (baglanti.State == ConnectionState.Closed)
            {
